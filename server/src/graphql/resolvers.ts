...
const pubsub = new PubSub<{ chatMessageAdded: ChatMessage }>();
const CHAT_MESSAGE_ADDED = "chatMessageAdded";
...
function assertScore(score?: number | null) {
	if (score != null && (!Number.isInteger(score) || score < 1 || score > 5)) {
		throw new GraphQLError("score must be an integer between 1 and 5.", {
			extensions: { code: "BAD_USER_INPUT" },
		});
	}
}
	...
	return {
		...
		pageInfo: {
			endCursor:
				visibleRows.length ?
					encodeCursor(
						visibleRows[visibleRows.length - 1].createdAt,
...
async function reviewConnection(bookId: string, args: CursorArgs) {
	...
	const rows = await prisma.review.findMany({
		where: {
			AND: [{ bookId }, cursorWhere(args.after)].filter(
				Boolean,
...
async function chatMessageConnection(bookId: string, args: CursorArgs) {
	...
	const rows = await prisma.chatMessage.findMany({
		where: {
			AND: [{ bookId }, cursorWhere(args.after)].filter(
				Boolean,
			) as Prisma.ChatMessageWhereInput[],
		},
		orderBy: [{ createdAt: "desc" }, { id: "desc" }],
		take: limit + 1,
	});
...
async function recommendedBooks(user: User | null, limit?: number | null) {
	...
	const rankedBooks =
		rankedIds.length ?
			await prisma.book.findMany({ where: { id: { in: rankedIds } } })
		:	[];
...
export const resolvers: Resolvers = {
	...
	Query: {
		...
		books: async (_: unknown, args: CursorArgs & { search?: string | null }) =>
			bookConnection(args),
		recommendations: async (
			_: unknown,
			{ limit = 5 }: { limit?: number | null },
			context: GraphQLContext,
		) => recommendedBooks(context.user, limit),
		chatMessages: async (_: unknown, args: CursorArgs & { bookId: string }) =>
			chatMessageConnection(args.bookId, args),
	},
	...
	Mutation: {
		...
		addReview: async (
			...
			{
				input,
			}: { input: { bookId: string; body: string; score?: number | null } },
			context: GraphQLContext,
		) => {
			const user = requireUser(context.user);
			...
			try {
				const review = await prisma.review.create({
					data: {
						bookId: input.bookId,
						userId: user.id,
						body: assertLength(input.body, "body", 5, 2000),
					},
				});
				if (input.score != null) {
				...
				throw new GraphQLError("Unable to add review for the requested book.", {
					extensions: { code: "BAD_USER_INPUT" },
				});
			}
		},
		sendChatMessage: async (
			_: unknown,
			{ input }: { input: { bookId: string; message: string } },
			context: GraphQLContext,
		) => {
			const user = requireUser(context.user);
			try {
				const chatMessage = await prisma.chatMessage.create({
					data: {
						bookId: input.bookId,
						userId: user.id,
						message: assertLength(input.message, "message", 1, 1000),
					},
				});
				...
				throw new GraphQLError(
					"Unable to send chat message for the requested book.",
					{ extensions: { code: "BAD_USER_INPUT" } },
				);
			}
		},
	...
	Subscription: {
		chatMessageAdded: {
			subscribe: async function* (_: unknown, { bookId }: { bookId: string }) {
				const iterator = (
					pubsub as unknown as {
						asyncIterator: (triggerName: string) => AsyncIterable<ChatPayload>;
					}
				).asyncIterator(CHAT_MESSAGE_ADDED);
				for await (const payload of iterator) {
					if (payload.chatMessageAdded.bookId === bookId) {
						yield payload.chatMessageAdded;
					}
				}
			},
		},
	...
	User: {
		...
		books: (user: User, _: unknown, context: GraphQLContext) =>
			context.loaders.booksByOwnerId.load(user.id),
	},
	...
	Book: {
		...
		owner: async (book: Book, _: unknown, context: GraphQLContext) =>
			requireEntity(
				await context.loaders.usersById.load(book.ownerId),
				"Book owner was not found.",
			),
		reviews: async (book: Book, args: CursorArgs) =>
			reviewConnection(book.id, args),
		ratings: (book: Book, _: unknown, context: GraphQLContext) =>
			context.loaders.ratingsByBookId.load(book.id),
	...
	Review: {
		...
		user: async (review: Review, _: unknown, context: GraphQLContext) =>
			requireEntity(
				await context.loaders.usersById.load(review.userId),
				"Review author was not found.",
			),
		book: async (review: Review, _: unknown, context: GraphQLContext) =>
			requireEntity(
				await context.loaders.booksById.load(review.bookId),
				"Review book was not found.",
			),
	},
	...
	Rating: {
		...
		user: async (rating: Rating, _: unknown, context: GraphQLContext) =>
			requireEntity(
				await context.loaders.usersById.load(rating.userId),
				"Rating author was not found.",
			),
		book: async (rating: Rating, _: unknown, context: GraphQLContext) =>
			requireEntity(
				await context.loaders.booksById.load(rating.bookId),
				"Rating book was not found.",
			),
	},
	...
	ChatMessage: {
		...
		user: async (message: ChatMessage, _: unknown, context: GraphQLContext) =>
			requireEntity(
				await context.loaders.usersById.load(message.userId),
				"Chat message author was not found.",
			),
		book: async (message: ChatMessage, _: unknown, context: GraphQLContext) =>
			requireEntity(
				await context.loaders.booksById.load(message.bookId),
				"Chat message book was not found.",
			),
	},
};
